...
def main():
    p = argparse.ArgumentParser(
        description="Download LROC (Lunar Reconnaissance Orbiter) images; thin wrapper over download_pds"
    )
    p.add_argument(
        "--volume", default="https://pds-imaging.jpl.nasa.gov/data/lro/XXDELETEME_lroc/edr/"
    )
    p.add_argument("--pattern", default=r"NAC.*\.IMG$")
    p.add_argument("--depth", type=int, default=3)
    p.add_argument("--max", type=int, default=20)
    p.add_argument(
        "--max-size-mb", type=float, default=12, help="skip files larger than this many MB"
    )
    p.add_argument(
