...
def raw_feed(category, num, page, search, newest):
    ...
    return (
        f"https://mars.nasa.gov/rss/api/?feed=raw_images&category={category}&feedtype=json"
...
def gallery_feed(category, num):
    q = urllib.parse.urlencode(
        {"feed": "images", "category": category, "feedtype": "json", "num": num}
    )
...
def main():
    ...
    p.add_argument(
        ...
        choices=sorted(set(RAW_CATEGORIES) | set(GALLERY_CATEGORIES)),
    )
    ...
    p.add_argument(
        "--camera", default="", help="camera search term, e.g. MCZ_LEFT, NAVCAM_LEFT, MAST"
    )
    ...
    if a.rover in RAW_CATEGORIES:
        ...
        for item in payload.get("images", []):
            ...
            photos.append(
                {
                    "img_src": files.get("full_res") or files.get("large") or files.get("medium"),
                    "title": item.get("title", ""),
                    "sol": item.get("sol", ""),
                    "camera": cam_obj.get("instrument", ""),
                    "earth_date": item.get("date_taken_utc", ""),
                }
            )
    else:
        ...
        for item in payload.get("imagegalleryitems", []):
            photos.append(
                {
                    "img_src": item.get("FULL") or item.get("MEDIUM") or item.get("BROWSE"),
                    "title": item.get("TITLE", ""),
                    "sol": "",
                    "camera": "",
                ...
                json.dump(
                    {
                        "rover": a.rover,
                        "sol": ph.get("sol", ""),
                        "camera": ph.get("camera", ""),
                        "earth_date": ph.get("earth_date", ""),
                        "src": src,
                    },
                    f,
                    indent=2,
