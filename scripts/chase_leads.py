...
SOURCE = {
    "PIA02442": dict(mission="Mariner 10", title="Moon North Pole mosaic",
                     type="imagery", product="Mariner 10 photomosaic (press product)",
                     url="https://images.nasa.gov/details/PIA02442"),
    "PIA12228": dict(mission="Cassini-Huygens", title="Water on the Moon (false-color map)",
                     type="map", product="Cassini VIMS/UVIS abundance map",
                     url="https://images.nasa.gov/details/PIA12228"),
    "PIA12229": dict(mission="Chandrayaan-1", title="Mineral Mapping the Moon (false-color map)",
                     type="map", product="M3 mineral map",
                     url="https://images.nasa.gov/details/PIA12229"),
    "PIA12233": dict(mission="LRO", title="Mapping the Moon, Point by Point (topography map)",
                     type="map", product="LOLA topography map",
                     url="https://images.nasa.gov/details/PIA12233"),
    "PIA12235": dict(mission="Chandrayaan-1", title="Nearside of the Moon mosaic",
                     type="imagery", product="M3 mosaic (press product)",
                     url="https://images.nasa.gov/details/PIA12235"),
    "PIA13227": dict(mission="LRO", title="The Earth from the Moon",
                     type="imagery", product="LROC WAC image (press product)",
                     url="https://images.nasa.gov/details/PIA13227"),
    "PIA13496": dict(mission="LRO", title="The Moon's Largest Impact Basin (SPA)",
                     type="imagery", product="LROC WAC SPA-basin mosaic; NAC detail M103196768LE",
                     url="https://images.nasa.gov/details/PIA13496"),
    "PIA13515": dict(mission="LRO", title="Natural Bridge on the Moon",
                     type="imagery", product="LROC NAC M113168034R (King crater melt sheet)",
                     url="https://images.nasa.gov/details/PIA13515"),
    "PIA13516": dict(mission="LRO", title="Moon seen from the East",
                     type="imagery", product="LROC WAC mosaic (press product)",
                     url="https://images.nasa.gov/details/PIA13516"),
    "PIA13517": dict(mission="LRO", title="Color of the Moon (color composite)",
                     type="map", product="LROC WAC color composite",
                     url="https://images.nasa.gov/details/PIA13517"),
    "PIA13642": dict(mission="LRO", title="Highest Point on the Moon",
                     type="imagery", product="LROC WAC mosaic (press product)",
                     url="https://images.nasa.gov/details/PIA13642"),
    "PIA13998": dict(mission="LRO", title="Challenger Astronauts Memorialized on the Moon",
                     type="imagery", product="LROC WAC mosaic of Apollo crater field",
                     url="https://images.nasa.gov/details/PIA13998"),
    "PIA14114": dict(mission="MESSENGER", title="The Moon as seen by MESSENGER",
                     type="imagery", product="MESSENGER MDIS full-disk view (press product)",
                     url="https://images.nasa.gov/details/PIA14114"),
    "PIA14208": dict(mission="MESSENGER", title="That No Moon... (Mercury)",
                     type="imagery", product="MESSENGER MDIS full-disk Mercury (press product)",
                     url="https://images.nasa.gov/details/PIA14208"),
    "PIA16621": dict(mission="GRAIL", title="Map of Moon Crust (false-color map)",
                     type="map", product="GRAIL crust-thickness map",
                     url="https://images.nasa.gov/details/PIA16621"),
    "PIA16622": dict(mission="GRAIL", title="GRAIL Gravity Tour (false-color map)",
                     type="map", product="GRAIL gravity map",
                     url="https://images.nasa.gov/details/PIA16622"),
}

# Independent-acquisition notes for imagery leads (from LROC/NASA publications).
INDEP = {
    "PIA13515": ("Natural bridge in King crater impact melt. LROC has imaged the "
    ...
    "PIA13496": ("South Pole-Aitken basin interior is cratered/ridged terrain; "
    ...
    "PIA14114": ("MESSENGER full-disk view of the Moon; the flagged region is the "
                 "bright lunar disk against black space (limb boundary), not a "
                 "surface feature."),
    "PIA14208": ("Full-disk Mercury image; flagged regions are limb/disk boundary "
                 "or albedo contrast, explained by geometry."),
    "PIA13642": ("LROC WAC mosaic of the highest-point region (Aristarchus plateau "
                 "area); small blobs are crater/ridge pixels."),
    "PIA13227": ("LROC WAC image of Earth above the lunar limb; flagged pixels near "
                 "the limb/Earth boundary."),
    "PIA13998": ("LROC WAC mosaic of the crater field used to memorialize the "
...
def product_source(pid):
    if pid.startswith(("ESP", "PSP")):
        return dict(
            mission="MRO", title="HiRISE browse product (%s)" % pid,
            type="imagery",
    ...
    if re.match(r"^M1?\d", pid):
        return dict(
            mission="LRO", title="LROC NAC browse product (%s)" % pid,
            type="imagery",
    ...
    if pid.lower().startswith("mars_") or "sol" in pid.lower():
        return dict(
            mission="Mars 2020 / MSL", title="Rover camera image (%s)" % pid,
            type="imagery",
            product="Perseverance/Mars rover camera EDR",
            url="https://mars.nasa.gov/mars2020/multimedia/raw-images/",
        )
    return dict(mission="unknown", title="unknown", type="unknown",
                product="unknown", url="")
...
def main():
    ...
    for n, r in enumerate(rows, 1):
        ...
        src = resolve_pid(pid)
        proc = r["path"]
        z, rawbox, raw_path = raw_z_score(proc, int(r["x"]), int(r["y"]),
                                          int(r["w"]), int(r["h"]))
        if z is None:
        ...
        if ftype == "map":
            verdict = ("NO-FINDING: false-color map product; flagged region is a "
                       "color/abundance boundary, not a surface feature.")
        elif pid in INDEP:
            verdict = ("NO-FINDING: real feature but fully explained; %s "
                       "(z=%.2f in original)." % (INDEP[pid].split(".")[0], z or 0))
        elif status == "BACKGROUND-LIMITED":
            verdict = ("NO-FINDING: feature collapses in the original product "
                       "(z=%.2f); explained as terrain/compression within a "
                       "press-processed JPEG.") % z
        elif status == "NO-RAW":
            verdict = "INCOMPLETE: raw product unavailable for verification."
        elif family == "edr":
            verdict = ("CANDIDATE: real surface feature that persists in the EDR "
                       "original (z=%.2f). Requires geolocation and an independent "
                       "acquisition at different lighting before classification.") % z
        else:
        ...
        indep = INDEP.get(pid, "No independent surface acquisition identified; "
        ...
        body = (
            ...
            pid, r["path"], src["title"], src["mission"],
            ftype, src["product"], src["url"], raw_hash,
            r["x"], r["y"], r["w"], r["h"],
            "(%s, %s, %s, %s)" % rawbox if rawbox else "unavailable",
            ("%.2f" % z) if z is not None else "unavailable", why,
            r["polarity"], r["contrast"], r["area_px"], r["aspect"], r["compactness"],
            r["persistence"], r["score"], r["interest"], indep,
            chk, verdict, conf,
            ("Geolocate this feature (PDS geometry / Trek) and check for an "
             ...
             "or albedo feature, not an unexplained anomaly." if is_candidate
    ...
    for fn, verdict, status, z in written:
        print("  %s  %-24s z=%s" % (os.path.basename(fn), status,
